<template>
    <el-main>
        ...
        <el-form ref="searchForm" label-width="80px" :inline="true" size="small">
            <el-form-item>
                <el-input v-model="searchModel.stockinName" placeholder="请输入库存物品名" size="small" />
            </el-form-item>
            <el-form-item>
                <el-input v-model="searchModel.empName" placeholder="请输入领取人姓名" size="small"/>
            </el-form-item>
            <el-form-item>
        ...
        <!-- 
         ...
         -->
        <el-table :data="tableData" border stripe style="width: 100%; margin-bottom: 20px" row-key="storeId"
            default-expand-all>
            ...
            <el-table-column prop="outIs" label="出库状态"/>
...
export default {
    ...
    methods: {
        ...
        async search(pageNo, pageSize) {
            ...
            //发送查询请求
            let res = await storeApi.getOutStockRecord(this.searchModel);
            //判断是否成功
            if (res.success) {
        ...
        async handleDelete(row) {
            ...
            if (confirm) {
                //发送删除请求
                let res = await storeApi.deleteRecord({ outId: row.outId });
                //判断是否发送成功
                if (res.success) {
                    //提示成功
                    this.$message.success(res.message);
     ...
     async handleDownload() {
        let confirm = await this.$myconfirm("确定要导出吗?")
            ...
            import('@/vendor/Export2Excel').then(excel => {
                const tHeader = ['物品名称', '出库数', '物品类型', '品牌','出库时间','领取人'] 
                const filterVal = ['stockinName', 'storeNum', 'stockinType', 'brand','stockoutTime','empName'] 
