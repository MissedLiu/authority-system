<template>
  <el-main>
    ...
    <el-table :data="tableData" border stripe style="width: 100%; margin-bottom: 20px" row-key="edId"
      ...
      <el-table-column prop="weixg.weixgName" label="维修人" />
      <el-table-column prop="state" label="状态" />
      <el-table-column label="操作" width="300" align="center">
        <template slot-scope="scope">
          <!-- <el-button icon="el-icon-edit-outline" type="primary" size="small" @click="handleToEm(scope.row)">
                      修理完成 </el-button> -->
          <el-button icon="el-icon-edit-outline" type="primary" size="small" @click="weixiu(scope.row)">
            分配维修工 </el-button>
    ...
    <system-dialog :title="toEmDialog.title" :visible="toEmDialog.visible" :width="toEmDialog.width"
      ...
      <div slot="content">
        <el-table ref="toEmTable" :data="toEmList" border stripe :height="toEmHeight"
          style="width: 100%; margin-bottom: 10px">
          <el-table-column prop="id" label="维修工编号" />
          <el-table-column prop="emp.empName" label="维修人" />
          <el-table-column label="操作" width="100" align="center">
            <template slot-scope="scope">
              <el-button icon="el-icon-edit-outline" type="primary" size="small" @click="xuanze(scope.row)">选择
              </el-button>
            </template>
          </el-table-column>
        </el-table>
...
*/
   ...
   async weixiu(row) {
        this.shuju.eeId=row.eeId;
      let res=await repairApi.findWeiXiu();
      if(res.success){
          this.toEmList=res.data;
      }
      //设置窗口标题
      this.toEmDialog.title = "分配维修人"
      //显示窗口
    ...
    //选择分配维修工事件
   async xuanze(row){
    this.shuju.weixgId=row.id;//维修工的登录账户id
    this.shuju.weixgName=row.emp.empName;
    let res=await repairApi.saveWeiXiuName(this.shuju);
    //判断是否发送成功
    if (res.success) {
          //提示成功
          this.$message.success(res.message)
